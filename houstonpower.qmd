...
crop_box <- st_transform(crop_box, crs = 3083)
census_tracts <- st_transform(census_tracts, crs = st_crs(crop_box))
census_cropped <- st_crop(census_tracts, crop_box)
...
night_light_colors <- c("0" = "#47497a", "100" = "#5f61a3","200" = "#787acf", "300" = "#d2d3f7", "400" = "#e4e5f5", "500" = "white")
...
We have two data frames we can plot to give us this information:
- `houses_blackout`, which contains geometries of census blocks that contain houses that experienced a blackout.
- `census_tracts`, which is just the census tracts geometries, that we can underlay to ensure the entire picture of Houston

```{r}
# Create plot using tmap
tm_shape(census_tracts, bbox = census_blackout) +      # Plot all census tracts
...
ggplot(data = houses_tracts, aes(x = blackout, y = median_income)) +
  geom_boxplot(aes(fill = blackout)) +                                                        # Specifying separate plots by blackout
  labs(title = "Median Incomes of Census Blocks",
       subtitle = "February 2021 Storms: Houston, TX",
       y = "Median Income") +
  scale_x_discrete(labels = c("FALSE" = "No Blackout", "TRUE" = "Blackout")) +                # Changing labels to be more intuitive
    theme_light() +
    scale_fill_manual(values = c("snow", "black"), labels = c("No Blackout", "Blackout")) +   # Customizing legend to fit labels
  theme(legend.title = element_blank())                                                       # Removing legend title
```

## Reflection

Summarize results and discuss limitations